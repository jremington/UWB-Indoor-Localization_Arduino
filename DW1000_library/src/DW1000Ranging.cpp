...
void DW1000RangingClass::loop() {
    // Überprüfen, ob ein Reset erforderlich ist
    checkForReset();
...
void DW1000RangingClass::handleSentMessage() {
    int messageType = detectMessageType(data);
    if (messageType != POLL_ACK && messageType != POLL && messageType != RANGE) return;

    if (_type == ANCHOR) {
        handleAnchorSentMessage(messageType);
    } else if (_type == TAG) {
        handleTagSentMessage(messageType);
    }
}

void DW1000RangingClass::handleAnchorSentMessage(int messageType) {
...
void DW1000RangingClass::handleTagSentMessage(int messageType) {
    if (messageType == POLL) {
        DW1000Time timePollSent;
        DW1000.getTransmitTimestamp(timePollSent);
        savePollTimestamp(timePollSent);
    } else if (messageType == RANGE) {
        DW1000Time timeRangeSent;
        DW1000.getTransmitTimestamp(timeRangeSent);
        saveRangeTimestamp(timeRangeSent);
    }
}

void DW1000RangingClass::handleRangeMessage() {
    DW1000Time timeRangeSent;
    DW1000.getTransmitTimestamp(timeRangeSent);
...
void DW1000RangingClass::handleReceivedMessage() {
    ...
    if (messageType == BLINK && _type == ANCHOR) {
        ...
        handleRangingInitMessage();
    } else {
        handleShortMACFrame(messageType);
    }
}

void DW1000RangingClass::handleBlinkMessage() {
    byte address[8];
    byte shortAddress[2];
...
void DW1000RangingClass::handleRangingInitMessage() {
    byte address[2];
...
void DW1000RangingClass::handleShortMACFrame(int messageType) {
    byte address[2];
    ...
    if (_type == ANCHOR) {
        handleAnchorReceivedMessage(messageType, myDistantDevice);
    } else if (_type == TAG) {
        handleTagReceivedMessage(messageType, myDistantDevice);
    }
}

void DW1000RangingClass::handleAnchorReceivedMessage(int messageType, DW1000Device* myDistantDevice) {
    ...
    if (messageType == POLL) {
        handlePollMessage(myDistantDevice);
    } else if (messageType == RANGE) {
        handleRangeReportMessage(myDistantDevice);
    }
}

void DW1000RangingClass::handlePollMessage(DW1000Device* myDistantDevice) {
...
void DW1000RangingClass::handleRangeReportMessage(DW1000Device* myDistantDevice) {
...
void DW1000RangingClass::handleTagReceivedMessage(int messageType, DW1000Device* myDistantDevice) {
    if (messageType != _expectedMsgId) {
        return;
    }
...
void DW1000RangingClass::savePollTimestamp(DW1000Time& timePollSent) {
    if (_lastSentToShortAddress[0] == 0xFF && _lastSentToShortAddress[1] == 0xFF) {
        for (uint16_t i = 0; i < _networkDevicesNumber; i++) {
...
void DW1000RangingClass::saveRangeTimestamp(DW1000Time& timeRangeSent) {
    if (_lastSentToShortAddress[0] == 0xFF && _lastSentToShortAddress[1] == 0xFF) {
        for (uint16_t i = 0; i < _networkDevicesNumber; i++) {
            _networkDevices[i].timeRangeSent = timeRangeSent;
        }
