...
// data for Excel .csv file
    ...
    printf("%6.2f, %6.2f, ",rc[0],rc[1]); //calculated and averaged r, given noisy measurements
    printf("%6.2f, %6.2f, ",rcavg2[0],rcavg2[1]); //point cluster axial SDs

    tmp = sqrt(rcavg2[0]*rcavg2[0]+rcavg2[1]*rcavg2[1]);
    printf("%6.2f, ", tmp);  //SD for calculated rc point cluster
